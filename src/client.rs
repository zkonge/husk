...
impl<R: Read, W: Write> TlsClient<R, W> {
    ...
    fn handshake(&mut self) -> TlsResult<()> {
        ...
        macro_rules! expect {
            ($var:ident) => {{
                match self.reader.read_handshake()? {
        ...
        let (key_data, pre_master_secret) = kex.compute_keys(&server_key_ex_data, &mut self.rng)?;

        expect!(server_hello_done);

        let client_key_exchange = Handshake::new_client_key_exchange(key_data)?;
        self.writer.write_handshake(&client_key_exchange)?;

        self.writer.write_change_cipher_spec()?;
        ...
        let msgs = {
            ...
            client_hello.tls_write(&mut msgs)?;
            Handshake::server_hello(server_hello_data).tls_write(&mut msgs)?;
            Handshake::certificate(certificate_list).tls_write(&mut msgs)?;
            Handshake::server_key_exchange(server_key_ex_data).tls_write(&mut msgs)?;
            Handshake::server_hello_done(DummyItem).tls_write(&mut msgs)?;
            client_key_exchange.tls_write(&mut msgs)?;
            msgs
        };
        ...
        let finished = Handshake::new_finished(client_verify_data)?;
        self.writer.write_handshake(&finished)?;

        // Although client->server is encrypted, server->client isn't yet.
        // server may send either ChangeCipherSpec or Alert.
        self.reader.read_change_cipher_spec()?;
        ...
        let server_finished = expect!(finished);
        {
            ...
            let verify_ok = crypto_compare(&server_finished, &server_verify_data);
            if !verify_ok {
    ...
    pub fn close(&mut self) -> TlsResult<()> {
        ...
        self.writer.write_alert(&alert_data)?;
        Ok(())
    }
